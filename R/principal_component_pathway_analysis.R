...
#' @param biplot a boolean to indcate whether biplot is included
...
principal_component_pathway_analysis = function(mass_matrix,
  ...
  require(Matrix)
  mass_matrix = Matrix(mass_matrix, sparse = T)
  ...
  # Runing PCA
  resampled_mat = Matrix(resampled_mat,
                         sparse =T)
  pca = prcomp(resampled_mat)
  print("PCA finished")
  gc()
  rm(mass_matrix)
  ...
  if(is.null(num_retained_component)){
    if(!is.null(variance_explained_threshold)){
      tryCatch(
        {
          ...
          plot(cumulative_variance, type = 'b', main = "Cumulative Variance Explained", xlab = "Number of Principal Components", ylab = "Cumulative Proportion of Variance Explained")
        ...
        error = function(cond) {
          stop("Check if correct variance threshold for principle components are inputted, should be numeric value between 0 and 1")
        },
      ...
      print("Both variance_explained_threshold and num_retained_component not inputted, use Kaiser's criterion for determination")
 ...
 # get principal components
  principal_components = mass_matrix %*% sorted_eigenvectors[, 1:retained]
  source("./network_based_pathway_ananlysis/runFisherTest.R")
  

  if(!is.null(input_mz)){
    if((length(input_mz)!= ncol(mass_matrix))|(!is.numeric(input_mz))){
      stop("Please ensure input_mz has one-to-one correspondence with each column of the mass_matrix")
    }else{
      input_mz = input_mz
    }
  }else{
    tryCatch(
      ...
      error = function(cond) {
       stop("Check whether column names of the input matrix is correctly labelled as the m/z ratio")
      },
      warning = function(cond) {
       stop("Check whether column names of the input matrix is correctly labelled as the m/z ratio")
      })
  ...
  #
  source("./QIMR_project/R/fct_db_adduct_filter.R")
  source("./QIMR_project/R/fct_formula_filter.R")
  source("./QIMR_project/R/fct_proc_db.R")
  #### Load the Cleaned and summarized DB ####
  Chebi_db     = readRDS("./QIMR_project/inst/db_files/Chebi_1_names.rds")
  HMDB_db      = readRDS("./QIMR_project/inst/db_files/HMDB_1_names.rds")
  ...
  if(is.null(ppm_error)){
    ...
    ppm_error_df = estimate_mz_resolution_error(mass_matrix = resampled_mat,
                                              ion_mode = ion_mode,
                                              matrix_molecule = matrix_molecule,
                                              tof_resolution=tof_resolution,
                                              plot = F)
    if(nrow(ppm_error_df) == 0){
      stop("No close metabolites find around given matrix mass, please specify a ppm_error, e.g 20")
    }else{
  ...
  #Set progress bar
  pb = txtProgressBar(min = 0, max = retained, initial = 0, style = 3)
  ...
  for(i in 1:retained){
    ...
    loading = data.frame(cbind(abs_loading = abs(pca[["rotation"]][,i]),
                    sign_loading = sign(pca[["rotation"]][,i]))) %>% arrange(desc(abs_loading))
    ...
    suppressWarnings({gsea_result = fgsea(pathways =  pathway_db,
                                            stats = ranks,
                                            minSize = 5,
                                            maxSize = 500) %>% filter(pval<=p_val_threshold) %>% mutate(principle_component = paste0("PC",i))%>%
